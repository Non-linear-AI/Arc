...
class TestEvaluatorServiceRefactor:
    ...
    def test_get_evaluators_by_model(self, evaluator_service):
        ...
        # Create multiple evaluators for the same model version
        eval1 = Evaluator(
            id="eval-001",
            name="evaluator-1",
            version=1,
            model_id="model-123-v1",
            spec="spec1",
        ...
        # Get evaluators for model-123-v1
        evaluators = evaluator_service.get_evaluators_by_model("model-123-v1")
        assert len(evaluators) == 2
        assert all(e.model_id == "model-123-v1" for e in evaluators)

        # Get evaluators for model-456-v1
        evaluators = evaluator_service.get_evaluators_by_model("model-456-v1")
        assert len(evaluators) == 1
        assert evaluators[0].model_id == "model-456-v1"
    ...
    def test_update_evaluator_model_id(self, evaluator_service, sample_evaluator):
        ...
        # Update model_id
        sample_evaluator.model_id = "new-model-v2"
        sample_evaluator.updated_at = datetime.now()
        evaluator_service.update_evaluator(sample_evaluator)

        # Verify update
        retrieved = evaluator_service.get_evaluator_by_id(sample_evaluator.id)
        assert retrieved.model_id == "new-model-v2"

    def test_list_all_evaluators_contains_model_id(
        self, evaluator_service, sample_evaluator
    ):
        ...
        assert our_eval.model_id == "test-model-v1"
    ...
    def test_get_evaluator_by_name_version_with_model_id(self, evaluator_service):
        ...
        retrieved = evaluator_service.get_evaluator_by_name_version(
            "unique-evaluator", 5
        )
        assert retrieved is not None
        assert retrieved.model_id == "model-xyz-v3"
    ...
    def test_evaluator_versioning_with_models(self, evaluator_service):
        ...
        assert all_versions[0].model_id == "model-b-v1"
        assert all_versions[1].version == 1
        assert all_versions[1].model_id == "model-a-v1"
    ...
    def test_no_trainer_id_in_evaluator(self, evaluator_service, sample_evaluator):
        ...
        # Verify model_id attribute exists
    ...
    def test_delete_evaluator_by_model(self, evaluator_service):
        ...
        evaluators = evaluator_service.get_evaluators_by_model("model-delete-v1")
        assert len(evaluators) == 1
