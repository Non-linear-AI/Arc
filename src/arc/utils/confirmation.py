...
class ConfirmationService:
    ...
    async def request_confirmation(
        ...
        # Check session flags
        if (self.session_flags["all_operations"] or
            (operation_type == "file" and self.session_flags["file_operations"]) or
            (operation_type == "bash" and self.session_flags["bash_commands"])):
    ...
    async def _show_confirmation_dialog(
        ...
        if content:
            console.print(f"  [dim]⎿ Requesting user confirmation[/dim]")
            lines = content.split('\n')
        ...
        while True:
            try:
                ...
                if choice_num == 1:
                    ...
                    elif "file" in operation.lower() or "create" in operation.lower() or "edit" in operation.lower():
