...
class DataSourceSpec:
    ...
    def validate_table_lifecycle(self) -> None:
        """Validate table lifecycle: check for drops followed by references.

        This simulates execution to catch logical errors like:
        - Dropping a table then trying to use it later
        - Inserting into a table that was dropped
        - Creating a table then dropping it before output

        Raises:
            ValueError: If table lifecycle issues are detected
        """
        # Get execution order first
        try:
            ordered_steps = self.get_execution_order()
        except ValueError as e:
            # Can't validate lifecycle if execution order is invalid
            raise ValueError(f"Cannot validate table lifecycle: {e}") from e

        # Track table state during simulated execution
        dropped_tables = set()  # Tables that have been dropped
        created_tables = set()  # Tables/views that have been created

        # Simple regex to extract table names from DROP statements
        drop_pattern = re.compile(
            r"DROP\s+(?:TABLE|VIEW)\s+(?:IF\s+EXISTS\s+)?['\"]?(\w+)['\"]?",
            re.IGNORECASE,
        )

        # Pattern to extract table names from INSERT/UPDATE/DELETE/CREATE statements
        ...
        for step in ordered_steps:
            ...
            # Check what this step does
            if step.type == "execute":
                # Check for DROP statements
                drop_matches = drop_pattern.findall(sql)
                for table in drop_matches:
                    dropped_tables.add(table)
                    # Remove from created set if it was created earlier
                    created_tables.discard(table)

                # Check for INSERT/UPDATE/DELETE (modifying tables)
                insert_matches = insert_pattern.findall(sql)
                update_matches = update_pattern.findall(sql)
                delete_matches = delete_pattern.findall(sql)

                for table in insert_matches + update_matches + delete_matches:
                    if table in dropped_tables:
                        raise ValueError(
                            f"Step '{step.name}' tries to modify table '{table}' "
                            f"which was dropped by an earlier step. "
                            f"Either remove the DROP statement or don't reference "
                            f"the dropped table."
                        )

                # Check for CREATE statements (recreating tables)
                create_matches = create_pattern.findall(sql)
                for table in create_matches:
                    # Recreating a table removes it from dropped set
                    dropped_tables.discard(table)
                    created_tables.add(table)

            elif step.type in ("table", "view"):
                # Table/view steps create tables
                created_tables.add(step.name)
                # Remove from dropped set if it was dropped earlier
                dropped_tables.discard(step.name)
